...
PD_CONSTRUCTOR static void gameConfigInit(void)
{
	...
	configRegisterInt("Game.MenuColourScheme", &g_MenuColourScheme, 0, 15);
	...
	for (s32 j = 0; j < MAX_LOCAL_PLAYERS; ++j) {
		...
		configRegisterInt(strFmt("Game.Player%d.CrosshairUniversal", i), &g_PlayerExtCfg[j].crosshairuniversal, 0, 1);
