...
3. **Perspective divide + screen map** on the CPU: `ndc = clip.xy / w`, then
   ...
   (Y flipped here — that's why the viewport stays identity). `dst[2] = clamp(ndc.z, 0,1)`.
...
- **Depth (works):** supply `ndc.z` in **[0,1]** (NOT pre-scaled); viewport Z-scale =
  `0xFFFFFF`, offset 0; **explicit `clip_min=0` / `clip_max=0xFFFFFF`** (leaving the clip
  range at pbkit's default crammed every fragment to one depth → no sorting); clear to
  `0xFFFFFF` (far) + `LESS_OR_EQUAL`. Bind position as **3 components when
  `depth_test`** (z reaches the Z24 buffer), 2 otherwise. Depth direction matches the GL
  backend.
...
- **`dst[2] = ndcz * 0xFFFFFF`** (pre-scaled depth): clamped to one value → no sorting.

## Current state (commit `f99d66acf`)

WORKING: boots to gameplay, controllable, missions load; textured (affine);
**hardware depth** sorts within a room; near-plane clip stops the stretching;
opaque walls solid (blend gate); 2D/HUD/menus correct; portal scissor implemented;
config persists; input; save paths.

OPEN / BROKEN:
1. **"Objects only visible against sky"** — props/chrs render at the **backmost** depth
   (fail depth vs any wall). Depth *direction* is correct, so suspect z values
   crushed/clamped to far for object draws. A diagnostic is **live**: `nxdk_start_frame`
   logs `rdr: FRAME N draws=… zmin=… zmax=… (x1000)` (NDC-z span ×1000 over depth-tested
   draws). **First action next session: read those values from a level-with-objects log.**
   Wide span (≈0…1000) ⇒ values fine, look at draw classification / `depth_source_prim`
   (G_ZS_PRIM, currently ignored in `nxdk_set_depth_mode`). Crammed (e.g. 985…1000) ⇒
   precision collapse, fix the z mapping / near plane.
2. **Affine texture warp** — textures swim (no perspective correction) because position
   ...
   real fix: revisit a **GPU-transform-from-clip-space** path (get the viewport right
   this time) so the HW does perspective-correct everything — but that's the risky path
   that failed twice; prototype carefully.
3. **Missing font letters** — some glyphs don't render (font atlas / UV-clamp issue,
   separate from depth).
4. **No sound** (audio subsystem not wired).
5. **Portal scissor + objects-at-back are both untested as of this writing** — confirm
   `f99d66acf` on hardware first.

## Next steps (priority order)

1. **Diagnose objects-at-back** from the `zmin/zmax` trace (see #1 above); fix z (or
   honour `depth_source_prim`). Highest gameplay impact.
...
3. **Perspective-correct textures** (de-warp) — design a clip-space GPU path or a verified
   projective setup; validate on one textured wall before rolling out.
4. **Fonts**, then **sound**, then the **perf/mem HUD** (FPS/CPU%/GPU%/mem vs the 64 MB
   budget — extends `pd.perf()` / `scripts/perf_overlay.lua`; see `docs/PORT_XBOX_NXDK.md`).
5. **Strip diagnostics** (`rdr:`/`PDBOOT:` traces, the `g_NxdkZMin/Max` span, per-frame
