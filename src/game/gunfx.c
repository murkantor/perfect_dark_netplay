...
Gfx *beamRender(Gfx *gdl, struct beam *beam, bool arg2, u8 arg3)
{
	...
	if (arg3 < 5 && beam->age >= 0) {
		...
		if (beam->weaponnum == WEAPON_LASER) {
			...
			colours[0].a = 150 + (rngCosmeticRandom() % 50);

			if ((rngCosmeticRandom() % 5) == 0) {
				colours[0].r = colours[0].g = 255 - (rngCosmeticRandom() % 100);
			}
		} else {
			sp130 = 30.0f;
		}
...
void casingCreateForHand(s32 handnum, f32 ground, Mtxf *mtx)
{
	...
	if (casing != NULL) {
		...
		if (weaponnum == WEAPON_PP9I || weaponnum == WEAPON_CC13
			...
			sp5c = ((s32)((rngCosmeticRandom() >> 24) * magic) >> 10) + magic;
			f0 = (rngCosmeticRandom() % sp5c) / PALUP((f32) (OS_CPU_COUNTER / 60));
			...
			sp4c = ((s32) ((rngCosmeticRandom() >> 24) * magic) >> 10) + magic;
			f0 = (rngCosmeticRandom() % sp4c) / PALUP((f32) (OS_CPU_COUNTER / 60));
