...
void sparkCreate(struct coord *pos, struct sparktype *type)
{
	...
	spark->speed.x = (s32)(rngCosmeticRandom() % (type->unk00 * 2 + 1)) - type->unk00;
	spark->speed.y = (s32)(rngCosmeticRandom() % (type->unk00 * 2 + 1)) - type->unk00;
	spark->speed.z = (s32)(rngCosmeticRandom() % (type->unk00 * 2 + 1)) - type->unk00;
	...
	if (type->unk18 % 2) {
		spark->ttl = rngCosmeticRandom() % type->maxage;
	} else {
...
void sparksCreate(s32 room, struct prop *prop, struct coord *pos, struct coord *arg3, struct coord *arg4, s32 typenum)
{
	...
	if ((typenum == SPARKTYPE_BLOOD || typenum == SPARKTYPE_FLESH) && prop && prop->type == PROPTYPE_CHR) {
		...
		type->unk1c = (rngCosmeticRandom() % 2 ? 0xff000000 : 0) | (rngCosmeticRandom() % 2 ? 0x00ff0000 : 0) | (rngCosmeticRandom() % 2 ? 0x0000ff00 : 0) | 0xff;

		if (type->unk1c == 0x000000ff) {
			type->unk1c |= 0x0000ff00 << ((rngCosmeticRandom() % 3) * 8);
		}

		type->unk20 = (rngCosmeticRandom() % 2 ? 0xff000000 : 0) | (rngCosmeticRandom() % 2 ? 0x00ff0000 : 0) | (rngCosmeticRandom() % 2 ? 0x0000ff00 : 0) | 0xff;

		if (type->unk20 == 0xff) {
			type->unk20 |= 0x0000ff00 << ((rngCosmeticRandom() % 3) * 8);
		}
	}
